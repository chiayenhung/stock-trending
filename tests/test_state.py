...
def test_logical_run_revision_is_idempotent(tmp_path: Path) -> None:
    ...
    first = store.create_or_resume(identity(), {"workflow": "3.0.0"}, 1)
    second = store.create_or_resume(identity(), {"workflow": "3.0.0"}, 1)
    assert first["run_id"] == second["run_id"]
    revised = store.create_or_resume(identity(), {"workflow": "3.0.0"}, 2)
    assert revised["run_id"] != first["run_id"]


def test_state_machine_rejects_invalid_transition(tmp_path: Path) -> None:
    store = RunStore(tmp_path)
    manifest = store.create_or_resume(identity(), {"workflow": "3.0.0"}, 1)
